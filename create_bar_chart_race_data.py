...
class CreateBarChartRaceData:
    ...
    def create_covid_19_confirmed(self):
        connection = sqlite3.connect("data/covid_19_2.db")
        sql_query = """
        SELECT reported_on,
               country,
               confirmed
    ...
    def create_covid_19_deaths(self):
        ...
        connection.close()
        nlargest_deaths_index = covid_19_deaths.groupby("reported_on")["deaths"].nlargest(10).reset_index()["level_1"].values
        covid_19_deaths = covid_19_deaths.loc[nlargest_deaths_index, :].reset_index(drop=True)
        return covid_19_deaths
    def create_covid_19_doses_administered(self):
        connection = sqlite3.connect("data/covid_19_2.db")
        sql_query = """
        SELECT reported_on,
              country,
          ...
          FROM time_series;
        """
        covid_19_doses = pd.read_sql(sql_query, con=connection)
        connection.close()
        nlargest_doses_administered_index = covid_19_doses.groupby("reported_on")["doses_administered"].nlargest(10).reset_index()["level_1"].values
        covid_19_doses = covid_19_doses.loc[nlargest_doses_administered_index, :].reset_index(drop=True)
...
print(covid_19_doses['doses_administered'].max()) 